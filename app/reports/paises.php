...
if (isset($_GET['id'])) {
    ...
    // Se instancia el modelo en este caso Pais para procesar los datos.
    $pais = new Pais;
    if ($pais->setId($_GET['id'])) {
        ...
        if ($rowPais = $pais->readOne()) {//leer un solo empleado
            ...
            if ($paiss = $pais->readReport()) {// leer todos los registros
                ...
                // Se establece la fuente para los datos de los paises.
                $pdf->Cell(100, 10, utf8_decode('Código postal'), 1, 0, 'C', 1);
                $pdf->SetFont('Arial', '', 11);
                $pdf->Ln();
                // Se recorren los registros
                foreach ($paiss as $rows) {
                    // Se imprimen las celdas con los datos de los paises.                    
